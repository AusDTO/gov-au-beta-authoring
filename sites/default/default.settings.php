...
if (isset($_ENV['VCAP_SERVICES'])) {
    ...
    foreach ($service_blob as $service_provider => $service_list) {
        foreach ($service_list as $some_service) {
            ...
            if (in_array('DTO-staging-RDS-mysql', $some_service['tags'], true)) {
                $mysql_services[] = $some_service;
                continue;
            }
            // look for a service where the label includes 'mysql'
            if (strpos($some_service['label'], 'mysql') !== false) {
                $mysql_services[] = $some_service;
            }
        }
    }
...
if (isset($_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR']) && file_exists($_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR'] . '/loc_govcms_7_x_2_0_beta9_core_dd.inc')) {
  require $_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR'] . '/loc_govcms_7_x_2_0_beta9_core_dd.inc';
}
// </DDSETTINGS>
